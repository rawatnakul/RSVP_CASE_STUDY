...
select count(*) as directors_map
from director_mapping;


select count(*) as movie_genres
from genre;

select count(*) as name_of_movies
from movie;

select count(*) as actor_names
from names;

select count(*) as movie_ratings
from ratings;


select count(*) as role_maps
from role_mapping;
...
SELECT 
    sum(total_votes) AS total_votes, languages
FROM
    movie AS m
        INNER JOIN
...
(
Select DISTINCT(name) as actress_name,
sum(total_votes) as total_votes,
count(title) as movie_count,
...
INNER JOIN movie AS m
ON r.movie_id = m.id
WHERE rm.category = 'actress' and m.country LIKE '%India%' and m.languages = 'Hindi'
...
INNER JOIN genre as g
on m.id = g.movie_id
WHERE rm.category = 'actress' and avg_rating >8 and genre ='Drama'
GROUP BY actress_name
)
SELECT *
FROM top_3_actress
